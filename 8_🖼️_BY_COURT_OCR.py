...
from utils import ensure_session, render_reset_button, parse_image_to_text, parse_stats_from_text
...
with c1:
    ...
    if upA:
        try:
            ...
            st.session_state.bycourt["A_raw"] = txt if ok else ""
...
with c2:
    ...
    if upB:
        try:
            ...
            img = Image.open(upB)
            txt, ok = parse_image_to_text(img)
            st.session_state.bycourt["B_raw"] = txt if ok else ""
...
if st.button("📥 Leggi (campi utili)", use_container_width=True):
    ...
    st.session_state.bycourt["A"] = pa["A"] or pa["B"]
    st.session_state.bycourt["B"] = pb["B"] or pb["A"]
